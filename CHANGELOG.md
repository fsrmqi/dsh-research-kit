...
- 📊 **科研路线可视化落地（新渲染链路，设计文档 `docs-internal/research-route-visualization.md`）**：① 新增 `scripts/render-diagrams.mjs`（零依赖、确定性渲染：单文件交互 HTML，含路线动画/搜索/深浅主题/哈希状态/`SRC` 事实出处标记/PNG 与 1200×630 分享卡导出）与 `scripts/validate-diagrams.mjs`（10 条规则码机器可读诊断 + supportedFixes，入 `npm run check` 门禁；diagram IR 契约含 source 证据链——path/sha256/anchor，结果文件变动自动标记图过期）；② 工作台新增**组装回放面板**：`composeWorkflow` 增量返回 `trace`（既有四个返回字段与 5 个调用点零改动），路线逐段点亮并与 Prompt 文本互证（测试强制每条 trace 都能在 Prompt 中定位），`prefers-reduced-motion` 与「切到静态」降级，快照导出主题色烘焙且不含播放运行时；③ 目录新增「研究结果解释图生成」工作流（可视化与图表 12 → 13）：支持会话产出与文件驱动（`--from-files` 脚手架）两条路径，综述场景分「检索逻辑/内容逻辑」两幕。目录工作流 316 → **317** 条（总资源 524 → 525）。动画机制参考 archify（MIT，只借鉴公开文档描述的范式与机制、不复制代码）；五张文档 SVG 维持现状（用户裁定不在范围内）。
