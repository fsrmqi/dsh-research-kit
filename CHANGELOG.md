...
- 🤖 **证据库一键 Agent 批量初判**：证据库新增批量判断入口，默认跳过已人工处理条目，可勾选后重新取得当前会话模型的最新结果。请求按最多 6 条分批，支持进度与取消；模型输出严格校验条目 ID 和证据维度枚举。Agent 判断写入独立的 `agentAssessment`，保留最近 5 次历史，不覆盖人工核验状态、人工依据或状态字段；写回前做并发快照检查，期间被修改或删除的条目会跳过。该初判只发送标题、链接、标识符、来源数据库、保存原因和笔记，不读取来源全文、不等同于 DOI/全文核验，结果仍需人工确认。

### 修复

- 🔧 **Agent 批量判断空响应处理**：前端不再直接对空响应调用 `Response.json()`；服务端也避免把正常响应关闭误判为客户端取消。异常时显示可读的 HTTP/模型路由提示，批处理可明确报告已完成与未处理数量。
- 🌐 **Agent 批量判断跨源预检**：批量判断路由现在接受 `OPTIONS` 预检并返回受限的 `POST` / `content-type` CORS 响应，避免插件页面与 DSH Web 服务跨源时在真正 POST 之前被 405 拦截。
- 📈 **Agent 批处理过程可见**：批量判断期间显示当前批次、已完成 / 总数、跳过、失败、进度条和最近一条写回结果，取消或失败时保留准确的处理统计。
- 🗂️ **主题分组改为多主题展开**：主列表按内容命中的全部主题归类；一条同时涉及植物、基因和测序的记录会出现在三个对应分组中，使用稳定的分组实例 key 避免重复渲染告警。平铺模式仍只显示一次。
...
- 🧭 **架构文档补上 4 个工作流新增字段的语义**：`scripts/add-agent-fields.mjs` 曾一次性为全部工作流补入 `input_schema` / `tool_mode` / `agent_guidance` / `checkpoints`，但这四个字段**在 ARCHITECTURE §4.2 的 schema 里从未出现**，也没进任何校验器或测试。现补入 §4.2，并写明三条容易误读的语义：`input_schema` 是 `placeholders` 的机器可读投影而非第二份真相；`tool_mode` **不改变执行方式**（插件从不执行工作流，它只是返回给宿主 Agent 的意图信号）；`agent_guidance` 与 `checkpoints` 只经 MCP 路径交付，界面路径不创建研究运行故没有检查点。同时如实标注这四个字段**目前不在目录校验范围内**，并把补校验登记为 [ROADMAP](ROADMAP.md) §12（含反向验证门槛）。

### 新增

- 📄 **新增《论文工作手册》与配套生命周期图**：[docs/PAPER-WORKFLOW.md](docs/PAPER-WORKFLOW.md) 补上此前缺失的**任务视角**入口——目录里有 36 条论文类工作流，但"我手上有一篇论文要写，从哪开始、按什么顺序走"没有一份文档回答。手册覆盖：九阶段全流程总表（每阶段该用哪条工作流、需不需要先给材料）、三条文献查询路径（插件直查的 5 个文献来源 / MCP 工具 / Agent 回退）及各自**能核验到什么程度**、一键流水线的四个入口阶段与八个人工闸门、MCP 侧从检索到审批检查点的最短调用链、以及需要显式确认的六类写操作。配图 `docs/assets/paper-lifecycle.svg` 画十阶段与八处必经闸门，两处强制完整性检查用琥珀底区分。图与文的事实全部取自 `catalog/workflows/` 与 `mcp/tool-registry.js` 实测，未凭工作流名称推测功能。
- 🧪 **文档统计门禁覆盖论文手册，并新增「工具摘要不得写死数量词」守卫**：`scripts/check-doc-stats.mjs` 新增 4 条规则（论文类工作流数 / 其中声明材料依赖的条数 / 文献研究工作流数与数据源数 / MCP 工具数），规则总数 36 → 40，新增的派生量按**类目**切分而非分片数——类目是读者在界面里筛选时看到的口径。另加一条通用守卫：`mcp/tool-registry.js` 的 `summaryZh` 中不得出现数量词，理由是注册表声明自己是纯数据模块、无法从 `catalog/` 或适配器清单派生规模（见「修复」节同一发现的漂移实例）。五项均为反向验证通过后才并入。
...
- 🧰 **MCP Server 与 Agent 活动面板（9/18 一批提交，此前未记入本文件）**：插件同时作为 MCP Server 提供 **31 个 `research_*` 工具**，宿主 Agent 可直接调用而无需经过 UI；[配置指南](docs/MCP-SETUP.md)给出 Codex / Claude Code / Zed / DSH 四种宿主的接入方式。
  ...
  - **运行状态与检查点**：`research_run_start` / `research_run_status` / `research_run_checkpoint_status|approve` / `research_run_export|import`；新增 `mcp/state/run-overview.js` 聚合同一次运行的阶段、检查点、证据、最近产物与推荐下一步，Agent 活动面板据此显示运行概览、支持产物按类型筛选，并可从 MCP 调用日志自动捕获 `run_id`。
  - **写回安全与可观测**：`research_evidence_save_batch` 一次性显式保存人工挑选的候选；`research_evidence_grade_apply` **默认只预览、确认后才写回**分级；`research_usage_stats` 报出调用次数 / 失败率 / 链路中断位置（仅脱敏元数据）。
  - **参数示例随注册表提供**：每个工具带 `example` 字段，配置指南的「最短路径」按任务组织并由注册表派生——调用示例不再与实现脱节。
- 🧭 **跨分区研究上下文（ROADMAP §11 基础设施第一片）**：新增 `src/research-context-store.js`——「当前项目 + 最小研究运行」在四个分区之间保持一致；工作流写入 / 发送 / 复制时创建**不含 Prompt 与原始内容**的运行记录，并与既有证据库项目选择同步，控制台导航显示当前项目与活跃运行。工作流 Prompt 带入格式受限的 `run_id` 协作指令，MCP 证据工具与运行导出可消费该可选关联键；`run_id` 不参与证据去重，也不把「写入输入框」表述为工具已执行。
- 🕸 **证据图谱交互增强**：节点数超过 50 或边节点比过低时自动切换稀疏图网格布局；迷你地图在节点密集时以聚合色块显示分布；证据节点新增详情卡（元数据 / 笔记 / 来源 / 删除，删除带确认与错误处理）与双击查看。
- ⚡ **浏览器端性能优化**：卡片使用 `content-visibility` / `contain-intrinsic-size`，搜索走 `useDeferredValue`，派生计算包进 `useMemo`，图谱拖拽改 `requestAnimationFrame`，证据刷新防抖，项目名输入延迟提交；移除调试用 `console.log`。
...
- 🧮 **修复工具注册表里写死的规模数字（其中一处已经漂移）**：`mcp/tool-registry.js` 的四处 `summaryZh` 把**本可派生、却无法在注册表内派生**的数量写成了字面量——「搜索 349 条工作流目录」「等 6 个数据源」「列出 8 个可用图表风格」「（支持 15 个期刊）」。注册表声明自己是「纯数据模块，无 Node 专属依赖」，因此没法像文档那样从 `catalog/` 或适配器清单实测，只能靠人记得同步；其中「8 个可用图表风格」**已经漂移**——`mcp/execution/figure-styles` 实际有 11 个（`bar_paired_delta` … `heatmap`），且该数字经 `scripts/sync-tool-docs.mjs` 流进了 [配置指南](docs/MCP-SETUP.md) 的工具表，而现有门禁只校验工具**数量**，看不见摘要内部。四处改为不含数量的措辞；规模信息没有损失——`research_catalog_search` 在调用时返回命中总数，`research_figure_list_styles` 与 `research_disclosure_list_policies` 本身就是清单入口。`scripts/check-doc-stats.mjs` 同步加一条通用守卫：`summaryZh` 出现「数字 + 个/条/种/篇/类」即失败，防止再被写回。
...
- 🔒 **材料护照 YAML 序列化加固**：`mcp/state/material-passport.js` 此前把 `project` / `current_stage` / `summary` 等字段不经引号直接拼进 YAML 标量，值里含换行或引号便能逃逸出所在字段、在护照文件里**注入新的顶层字段**。现统一以 JSON 字符串（YAML 双引号标量的安全子集）序列化全部用户可控字段，读取端仍兼容历史护照中未加引号的标量；`run_id` 本就受 `validRunId` 字符白名单约束，输出格式保持不变。已附安全性测试。
