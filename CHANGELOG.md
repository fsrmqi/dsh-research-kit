...
- 📊 **科研路线可视化落地（新渲染链路，设计文档 `docs-internal/research-route-visualization.md`）**：① **整体引入 archify 交互 viewer**（[tt-a1i/archify](https://github.com/tt-a1i/archify) MIT：`vendor/archify/template.html` + `i18n.mjs`，SHA 锁定于 vendor-manifest.json，归属见 NOTICE）——视觉预设（classic/signal-flow/blueprint/editorial）、章节故事（≤5 幕）、语义透镜、雷达总览、彗星流光路线动画原样获得，zh-CN 原生支持；适配层 `src/lib/archify-adapter.js` 把本仓库 IR/trace 转为其 `data-*` 契约；`scripts/render-diagrams.mjs --html` 按此产出单文件交互解释图（`SRC` 事实出处/分享卡导出随 viewer 提供）， `scripts/validate-diagrams.mjs`（10 条规则码机器可读诊断 + supportedFixes，入 `npm run check` 门禁；diagram IR 契约含 source 证据链——path/sha256/anchor，结果文件变动自动标记图过期）；② 工作台**组装回放改为弹出独立 archify viewer 窗口**：`composeWorkflow` 增量返回 `trace`（既有四个返回字段与 5 个调用点零改动），弹窗逐段回放本次组装事实并与 Prompt 文本互证（测试强制每条 trace 都能在 Prompt 中定位），面板内保留紧凑版（`prefers-reduced-motion` 与「切到静态」降级）；③ 目录新增「研究结果解释图生成」工作流（可视化与图表 12 → 13）：支持会话产出与文件驱动（`--from-files` 脚手架）两条路径，综述场景分「检索逻辑/内容逻辑」两幕。目录工作流 316 → **317** 条（总资源 524 → 525）。vendored 工件与归属声明见 `NOTICE`；五张文档 SVG 维持现状（用户裁定不在范围内）。
