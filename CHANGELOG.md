...
- 测试扩充至 122 项：新增「目录标注的插件可直查与实现里的适配器完全一致」（先锁住清单提取结果，避免一致性断言空转）、「契约校验能抓住漏标与虚标」、§4c 的 5 项守护——未选择不注入（含空数组 / `null` / 非数组 / 无参调用）、宿主不支持写入时不注入但引用块仍可复制、有选择时写入文本含来源链接与核验边界、视图确实经由 `planCitationWrite` 且 `setDraft` 受 `action === 'write'` 守卫（否则把守卫从视图删掉，纯逻辑测试仍会全绿）、选择基准必须是全部条目而非当前筛选结果——以及 §4d 的「已保存证据按来源库与查询来源接入图谱、且不暴露笔记」守护。
