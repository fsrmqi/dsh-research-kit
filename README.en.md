...
| Research Vault | Deposit | Asset CRUD, version diff, verification tracking, **explicit asset–evidence links**; an **Evidence Vault** sub-module saves source metadata and notes per item, project-isolated and de-duplicated, with a read-only "cited by" reverse lookup |
...
- **Deposit companion button** — a small round button next to the draft-enhancer launcher: without enabling auto deposition, one click deposits the most recent assistant reply of the current session (same extraction chain, everything starts as "to verify"); results show in a transient status chip.
...
- **Plugins-page status** — the Research Kit detail page reports Web / Shell / FS / LLM composition facts, connected MCP servers, and the number of direct-query adapters. It reports deployment facts only and never marks a catalog source as verified and available.
- **Live query settings** — `memoryServer`, `memoryTimeoutMs`, `databaseTimeoutMs`, `databaseRequestsPerMinute`, and `allowAgentFallback` are volatile Config fields editable on the Plugins page; they apply on the next request. Defaults are Memory Center, 15 seconds, 15 seconds, 12 requests per minute, and Agent fallback enabled.
...
- **Evidence vault** — save individual results from public database lookups, each carrying a stable identifier (DOI / PMID / NCT / arXiv) and source link, defaulting to "unverified"; project-isolated de-duplication, JSON export/import and hard delete. **Only source metadata and notes you write are stored — no full text, no query terms. Nothing is saved automatically except by the explicitly enabled auto deposition below, which tags every entry and keeps it "unverified".** The vault also provides one-click **Agent batch assessment**: by default it processes only entries without a human assessment; an opt-in checkbox reruns the current session model on human-assessed entries as well. Agent results are stored separately with a short history and never advance verification status or overwrite human notes. Only title, link, identifier, source database, save reason and note are sent to the model; no source full text is fetched, so results still require human verification.
...
- **Asset–evidence links** — an asset card expands into an "evidence links" panel: candidates derived from same project, shared tags and auto-deposition knowledge chains; **links are created only after explicit checkbox confirmation** (never automatically). Evidence entries show a read-only "cited by" list; the graph draws "asset → supporting evidence" edges (scoped to "persisted", dropped when an endpoint disappears). Links are not included in JSON backups yet (the export notice says so).
...
Verified so far: catalog contract validation (587 entries, 587 unique IDs, shards identical to the aggregated entries) and 418 regression tests in 61 files. Existing Web behavior previously passed real DSH web-profile smoke tests. The App transfer entry, export, and preview were verified in a real Desktop profile; final import and cross-Web migration still require real-App acceptance.
...
npm test        # pure-logic, contract and render-level regression tests (418 / 61 files)
