...
test('渲染器：--html 字节确定、动画锚点与免责声明就位、路径可被 --html 消费', () => {
  ...
  assert.match(html, /data-animation="trace"/, '缺少 archify 路线动画声明')
  assert.match(html, /archify-diagram-title/, '缺少 archify 无障碍标题')
  assert.match(html, /archify-guided-views-data/, '缺少章节故事数据槽位')
  assert.match(html, /data-preset="classic"/, '缺少视觉预设')
  assert.match(html, /archify-i18n-data/, '缺少 i18n 数据槽位')
  assert.match(html, /c-backend|c-frontend|c-database/, '节点应使用 archify 语义角色配色')
  assert.match(html, /prefers-reduced-motion/, '缺少动效降级分支')
  assert.match(html, /不代表运行时已验证/, '缺少免责声明')
  assert.ok(!html.includes('__METRICS__'), '实测计数应烘焙进节点标签而非注入运行时')
...
test('渲染器：文本注入被转义，导出按钮与哈希解析就位', () => {
  ...
  assert.match(html, /Export/, '缺少 archify 导出菜单')
  cleanup()
})
