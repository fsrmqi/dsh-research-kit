...
发布是**不可逆操作**：npm 只在极窄条件下允许撤回（限时，且要求无下游依赖、下载量低于阈值），超出后只能 `npm deprecate` 打废弃标记。因此发布前的第 0 步是确认「要发的就是这个版本」。

### 7.1 一次性前置

- npm 账号并开启 2FA（发布时需输入一次性验证码）：`npm login`；
- 包名 `dsh-research-kit` 为未加 scope 的公开包，发布后默认 public，无需 `--access`；
- 确认包名仍未被占用：`npm view dsh-research-kit version`——返回 `E404` 表示可用。

### 7.2 发布流程

```bash
# 1. 工作区必须干净：先提交所有改动（registry 上的版本对不上工作区是最难查的一类问题）
git status --short

# 2. 提升版本号（同时产生 commit 与 git tag；已发布过的版本号不能复用）
npm version patch   # 或 minor / major

# 3. 预演：打印将要上传的完整文件清单与体积，并触发下面的闸门
npm publish --dry-run
...
**`prepublishOnly` 闸门**：`package.json` 已把 `npm run check && npm test` 挂在 `prepublishOnly` 上，`npm publish` 会先跑完契约校验与全部测试才上传。这不是形式——`ui/client.js` 是提交进仓库的构建产物，若源码改了却忘记 `npm run build`，没有这道闸就会把一个与源码不一致的产物发到 registry 上，且该版本号从此不可复用，只能靠发新版本补救。

### 7.3 包内容由 `package.json` 的 `files` 白名单决定

上传内容不靠 `.npmignore` 推断，而是 `files` 显式声明：`catalog/`、`src/`、`ui/`、`dsh/`、`vendor/`、`scripts/`、`docs/`、`test/` 与各根级 md / `LICENSE` / `cordis.patch.yml`。因此 `docs-internal/`、`.workbuddy/`、`browser-results/` 等本机与过程目录天然不会进包。

判断某个文件是否真的会进包，**不要靠读 `files` 推断——跑一次 `npm publish --dry-run` 看清单**（它会逐文件列出）。新增顶级目录时必须同步 `files`，否则该目录整块静默丢失，而在本地开发中完全看不出来。

体积上，包内两个大件是 `ui/client.js` 与 `vendor/`（后者含 promptkit 与 archify 两个 vendored 工件）——它们占绝大部分体积且都属必需，不要为"瘦身"删减：删掉任何一个都会让插件在目标 profile 上无法渲染。

### 7.4 发布后

- **同步文档**：README（中英）的安装段与「兼容性与状态」段仍写着"尚未发布到 npm"，需补上 npm 通道作为推荐安装方式；`CHANGELOG.md` 记发布条目；
- **DSH 安装通道**：发布后可省略 clone 与 tarball 两条路径，直接装：
  ```bash
  dsh plugin --profile web add dsh-research-kit
  ```
  仍建议在 README 保留钉 commit 的 GitHub 路径，供需要可复现构建的用户使用；
- **第三方插件站的信息会滞后**：插件市场抓的是仓库某个 commit 的 README 快照，发布后其展示内容可能仍停留在旧版本，不以其为准。
