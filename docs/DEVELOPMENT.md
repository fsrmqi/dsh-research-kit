...
当前仓库处于**可用状态**——587 项目录资产、统一视图四分区、公开数据源直查、方法工坊与草稿增强器、MCP Server（32 个 research_* 工具）与 Agent 活动面板均已实现，并通过两轮真实 DSH profile 烟测。本文描述**现状**与必须遵守的约束；未完成事项见根目录 [ROADMAP.md](../ROADMAP.md)。
...
npm test        # 先重建浏览器产物，再运行目录逻辑、分片聚合、存储层、查询、渲染级降级、组装回放、构建产物、分区契约与 DSH 槽位注册测试（367 项 / 46 个测试文件）
...
**本项无法由单元测试替代。** 仓库内 367 项测试覆盖纯逻辑断言、渲染级初始状态与源码/构建产物的文本断言（`test/dsh-slots.test.js` 直接调用注册表、slots 服务为模拟对象），能证明「产物能注册槽位」「降级时按钮真的带 disabled」，但不能证明目标 DSH 版本的 props 形状与之一致。
