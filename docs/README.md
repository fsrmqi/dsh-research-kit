...
对外展示用的图放在 `assets/`，全部是手写 SVG，随仓库分发、不经过构建：
...
3. **自动化测试无法替代真实 profile 验收。** 仓库内 377 项测试（47 个测试文件）覆盖纯逻辑断言、渲染级初始状态（真实 react-dom/server）与源码/产物文本断言，但交互（点击 / 事件）与宿主 props 形状仍只有真实 profile 能证明；升级 DSH 后必须重跑 [MANUAL-QA.md](MANUAL-QA.md)。
