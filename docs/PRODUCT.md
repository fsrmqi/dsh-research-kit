...
DSH 侧提供任务启动体验与高质量提示词资产；同包也提供可独立接入兼容宿主的 stdio MCP 工具集。它不托管模型、密钥或后台任务队列；模型调用、文件读取与会话执行仍由宿主承担。
...
独立 MCP 入口：兼容宿主 → `dsh-research-kit-mcp` → 文献、证据、审阅、研究运行工具 → 本地受控状态
```
...
- 不声称内置或自动接通外部学术数据库；
- 不实现自带模型路由、成本账本、持久任务队列或沙箱的独立 Agent 运行时；MCP 仅提供本地 stdio 工具服务；
- 不将模型输出自动认定为证据、可复现结果或经同行评审的结论；
- 不绕过 DSH 的模型、权限、MCP 或文件引用机制。

「数据库」和「技能」条目本质是**可见的元数据与提示词指导**。129 个数据源中有 11 个由插件内置适配器直接查询（目录状态标记为 `available-in-plugin`），其余 118 个如实标注接入前提——需要 MCP、订阅、API Key 或数据使用协议——并显式回退到当前会话的 Agent。**无论走哪条路径，插件都不会伪造检索结果。**

目录条目的接入前提由四档标注表达，每一档各自的界面行为与保证机制如下：

![数据源可用性的四档标注：从插件直查到尚未实现的宿主声明，每档各自的界面行为与保证机制](assets/availability-ladder.svg)

同一份标注决定条目走哪条查询路径——插件直查，或回退到当前会话的 Agent：
...
工作流按流程族分 28 个类目（论文与手稿、文献研究、数据分析、基因组学、临床研究、生态学、天文与空间科学等，明细见 [README「目录内容」](../README.md#目录内容)），数据源按 11 个研究入口分组（文献与引文、临床与公共卫生、基因组与遗传变异、组学与表达数据、蛋白质/结构/通路、化学/药物/毒理、天文与空间科学、生物多样性与生态、气候/地球/环境、地理空间与社会数据、材料与物理科学，另有兜底组「其他研究数据源」）。目录规模以质量优先，不以数量为目标。
...
- **目录与启动器**：587 项资源（349 工作流 + 109 技能 + 129 数据源，按流程族/用途分片维护）、参数替换、Prompt 预览/编辑、写入/发送、`@文件` 材料提醒、资源关联与不可用能力提示；
...
- **组装回放（分区①）**：启动工作流后逐段回放 `composeWorkflow` 的**真实组装事实**（工作流 → 参数 → 附加技能 → 数据源边界 → 人工核验），每段都能在生成的 Prompt 文本中定位到对应锚点，回放与正文互证；详情栏内嵌紧凑回放，「弹出回放窗口」打开独立 archify viewer（视觉预设 / 章节叙事 / 语义透镜 / 雷达总览）。回放只呈现**已发生的组装动作**，不预测执行结果，也不代表任何工具已执行。
- **研究结果解释图**：目录新增「研究结果解释图生成」工作流，配合 `scripts/render-diagrams.mjs` 把 diagram IR **确定性**产出为单文件交互 HTML（`--html`），或从会话产出文件生成 IR 脚手架（`--from-files`）；`scripts/validate-diagrams.mjs` 提供机器可读的规则诊断并进入 `npm run check`（仓库内不预置 IR 文件，校验在产出 / 审阅 IR 时生效）。
