...
「数据库」和「技能」条目本质是**可见的元数据与提示词指导**。82 个数据源中有 11 个由插件内置适配器直接查询（目录状态标记为 `available-in-plugin`），其余 71 个如实标注接入前提——需要 MCP、订阅、API Key 或数据使用协议——并显式回退到当前会话的 Agent。**无论走哪条路径，插件都不会伪造检索结果。**
...
- **目录与启动器**：299 项资源（206 工作流 + 11 技能 + 82 数据源，按流程族/用途分片维护）、参数替换、Prompt 预览/编辑、写入/发送、`@文件` 材料提醒、资源关联与不可用能力提示；
