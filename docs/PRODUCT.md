...
- 宿主动作缺失降级路径的自动化测试（五处守卫清单见 [ROADMAP §1](../ROADMAP.md)；**前置障碍是仓库尚无可在 Node 挂载 React 的渲染测试 harness**，需先引入或搭建，当前覆盖只到决策层）；
- 发送前自动增强（依赖宿主下发钩子）；
- **专用科研证据 Viewer**：确定性布局、缩放平移、聚焦/上下游/路径模式、迷你地图、主题与脱敏静态导出；当前基础 SVG 图谱不等同于该 Viewer，详细范围与验收见 [ROADMAP §3.1](../ROADMAP.md#31-专用科研证据-viewer)。
